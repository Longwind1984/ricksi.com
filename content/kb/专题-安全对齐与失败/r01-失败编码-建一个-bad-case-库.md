...
- **商业模式：库的字段决定它能不能进合规审计。** 当监管（EU AI Act 分阶段实施中、美国 2024 年 45 州近 700 个 AI 法案）要求企业证明"已知风险已被识别和缓解"时，一个有 `failure_type`/`fix_lever`/`legal_outcome` 字段的结构化库，就是审计证据；一个 Slack 频道则什么都证明不了。库的 schema 设计，本质是合规可审计性的前置投资。
