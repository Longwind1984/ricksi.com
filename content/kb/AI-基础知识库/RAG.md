...
LLM 是负责逻辑计算的 CPU，RAG 是外挂的硬盘与实时内存。企图用微调将知识压入模型权重中，在工程上极其低效且容易引发 [灾难性遗忘](/kb/ai-基础知识库/灾难性遗忘/)。

## 为什么不是用微调或长上下文？

| 方案 | 何时用 | 致命短板 |
|------|--------|---------|
| **SFT 注入知识** | 风格 / 行为塑形 | 难以更新；引发 [灾难性遗忘](/kb/ai-基础知识库/灾难性遗忘/)；事实仍可能幻觉 |
| **长上下文（百万 token）** | 单次会话内的大文档分析 | Prefill O(N²) 算力爆炸；"中间迷失" (Lost in the Middle) |
| **RAG** | 大语料知识库 + 频繁更新 + 可追溯 | 检索质量天花板限制最终生成质量 |

详细决策矩阵见 [c04 RAG vs 微调](/kb/ai-基础知识库/c04-模型训练全阶段-pipeline/)。
...
| 密集向量检索（[Embedding](/kb/ai-基础知识库/embedding/)） | 语义匹配；跨语言；模糊 query | 低频罕见词 Embedding 失真；黑盒 |
...
- **Agentic RAG**：检索作为 [Agent](/kb/ai-基础知识库/agent/) 工具之一；模型自行决定何时检索、检索什么、是否多次迭代检索。
...
- [c04 RAG vs 微调决策](/kb/ai-基础知识库/c04-模型训练全阶段-pipeline/)
- [c09 RAG 架构详解（工程版）](/kb/ai-基础知识库/c09-rag-架构/)
- [c13 RAG 作为幻觉应对策略](/kb/ai-基础知识库/c13-幻觉的不可消除性/)
- [Embedding 向量嵌入](/kb/ai-基础知识库/embedding/)（密集检索的底层技术）
- [Tokenization](/kb/ai-基础知识库/tokenization/)（影响检索精度）
- [灾难性遗忘](/kb/ai-基础知识库/灾难性遗忘/)（RAG 不引发遗忘，优于 SFT 注入知识）
- [Function Calling](/kb/ai-基础知识库/function-calling/)（Agentic RAG 的工具调用基础）
