...
关键前提：你需要一个**带 ground-truth 的标注集**——每条输出有 s(x)，也有"人工判定对/错"的标签。没有这个标注集，整条曲线无从画起，这也是本实验和 0412 评测专题 的硬连接：**校准评测产出的可靠性图（reliability diagram）和标注集，正是 auto-accept 实验的输入**。复用 0412 的校准 pass，不要另起炉灶。
