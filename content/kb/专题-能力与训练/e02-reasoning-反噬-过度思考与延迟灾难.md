...
最锋利的单一数字：Phi-4-reasoning 在常规任务上平均生成 **6,780 个推理 token**，而标准 Phi-4 只需 **378 个**——多了近 18 倍——但准确率反而**更低**。〔注：token 对比数字来自简报接地证据；Phi-4-reasoning 与 over/underthinking 框架的两篇相关论文为 arXiv:2504.21318（Phi-4-reasoning Technical Report，已核实存在）与 arXiv:2505.00127（"Between Underthinking and Overthinking"，已核实存在）；6,780 vs 378 的精确归属页码及"准确率更低"的具体分数标〔待核实〕，正文只取方向性结论。〕这条与 arXiv:2505.00127（"Between Underthinking and Overthinking: An Empirical Study of Reasoning Length and Correctness in LLMs"）的核心结论一致：模型**误判题目难度**，对简单题 overthink、对难题 underthink，无法按难度校准推理长度。
...
- 真实反例：Phi-4-reasoning 在常规任务上 6,780 token 换来低于标准版的准确率（arXiv:2505.00127）——"更聪明"在简单题上是负的。
