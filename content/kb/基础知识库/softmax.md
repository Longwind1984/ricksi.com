...
**Softmax 是把一组实数（logits）变成和为 1 的概率分布的函数**——它是 LLM 输出层、[Attention](/kb/基础知识库/attention/) 权重、[MoE](/kb/基础知识库/moe/) Router 三处共用的核心数学组件，几乎所有"哪个候选项更可能"的选择都通过它。
...
这就是 [幻觉](/kb/基础知识库/幻觉/) 的数学根源之一：**softmax 没有"我不知道"这一项**。它强制把概率全部分配给 vocab 内 token，没有 abstain 选项。
...
[MoE](/kb/基础知识库/moe/) 路由是 Top-K Softmax 最大规模的工程实践——每个 token 只激活 1-8 个 expert，背后是稀疏路由 + 负载均衡的精妙工程。
...
- [c03 Attention 中的 softmax](/kb/基础知识库/c03-transformer-核心机制与注意力变体/)
- [c08 Temperature / Top-K / Top-P](/kb/基础知识库/c08-解码策略与生成控制/)
- [c13 幻觉的数学根源](/kb/基础知识库/c13-幻觉的不可消除性/)
- [c14 校准与置信度](/kb/基础知识库/c14-模型评估体系与-goodhart-陷阱/)

## 关联节点

- 数学组件：[Attention](/kb/基础知识库/attention/) [Embedding](/kb/基础知识库/embedding/)
- 解码相关：[幻觉](/kb/基础知识库/幻觉/) [Test-Time Compute](/kb/基础知识库/test-time-compute/)
- 工程实现：[MoE](/kb/基础知识库/moe/) [自回归生成](/kb/基础知识库/自回归生成/)
