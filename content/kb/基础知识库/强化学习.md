...
**强化学习是通过"试错 + 奖励信号"让策略 π 在环境中逐步改善的学习范式**。LLM 时代它被三次重新发明：一次用来对齐（[RLHF](/kb/基础知识库/rlhf/)），一次用来推理（[System 2](/kb/基础知识库/test-time-compute/)），一次用来扩展数据（[合成数据](/kb/基础知识库/合成数据/) + Self-Play）。
...
- **目的**：让模型自己产生高质量训练数据
- **奖励信号**：自评 / 多模型投票 / 任务完成度
- **典型场景**：[合成数据](/kb/基础知识库/合成数据/) 闭环、多 Agent 对抗采样、Rejection Sampling Fine-tuning（RFT）
...
**根本困境**：RL 的优化压力会找到 RM 的所有漏洞——RM 永远不可能完美，所以 RL 训练不可能"训到收敛"，而是"训到刚好够用就停"。Goodhart 定律的直接体现，见 [c14 - 模型评估体系与 Goodhart 陷阱](/kb/基础知识库/c14-模型评估体系与-goodhart-陷阱/)。
...
- [c04 RLHF/PPO 工程链路](/kb/基础知识库/c04-模型训练全阶段-pipeline/)
- [c11 Reasoning via RL](/kb/基础知识库/c11-system-2-思维与-test-time-compute/)
- [c14 Goodhart 与 reward hacking](/kb/基础知识库/c14-模型评估体系与-goodhart-陷阱/)
- [c15 后训练 RL 与数据壁垒](/kb/基础知识库/c15-数据墙与后训练霸权/)

## 关联节点

- 算法：[RLHF](/kb/基础知识库/rlhf/) [Constitutional AI](/kb/基础知识库/constitutional-ai/) [DeepSeek](/kb/ai-公司与产品/deepseek/)
- 概念：[合成数据](/kb/基础知识库/合成数据/) [Test-Time Compute](/kb/基础知识库/test-time-compute/) [幻觉](/kb/基础知识库/幻觉/)
