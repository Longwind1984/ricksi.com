...
| 3.0 推理时扩展 | OpenAI o1/o3、DeepSeek-R1 2024+ | 推理时投入更多算力（[Test-Time Compute](/kb/基础知识库/test-time-compute/)）也遵循 scaling law | 算力可在「训练时」和「推理时」两条曲线间动态分配 |
...
- **数据墙**：当高质量人类文本接近耗尽，单纯加 D 不再产生预期的 loss 下降（见 [c15 数据墙](/kb/基础知识库/c15-数据墙与后训练霸权/)）
- **能力涌现非平滑**：某些能力（如算术、CoT 推理）呈阶跃式出现，不是连续幂律——这给 PM 的能力预测带来不确定性
- **Goodhart 陷阱**：loss 下降不等于真实能力提升；某些 benchmark 饱和后 loss 仍降但用户感受无变化（见 [c14](/kb/基础知识库/c14-模型评估体系与-goodhart-陷阱/)）
...
- [c04 预训练与 Scaling Laws](/kb/基础知识库/c04-模型训练全阶段-pipeline/)
- [c14 Benchmark 通胀](/kb/基础知识库/c14-模型评估体系与-goodhart-陷阱/)
- [c15 数据墙](/kb/基础知识库/c15-数据墙与后训练霸权/)
- [c11 算力转移](/kb/基础知识库/c11-system-2-思维与-test-time-compute/)

## 关联概念

[预训练](/kb/基础知识库/预训练/)·[合成数据](/kb/基础知识库/合成数据/)·[Test-Time Compute](/kb/基础知识库/test-time-compute/)·Chinchilla·数据墙·范式
