...
## 与 [KV Cache](/kb/基础知识库/kv-cache/) 的本质关系

自回归推理的成本 = 模型权重读取 + KV Cache 读取。两者都是 memory-bound：
- 短上下文：权重读取主导
- 长上下文：KV 读取主导（超过约 32K token 后）

→ Decode 阶段所有优化（[KV Cache 压缩](/kb/基础知识库/kv-cache/) / [Prompt Caching](/kb/基础知识库/prompt-caching/) / continuous batching）本质都在攻击同一个目标：**让 memory 带宽不要成为天花板**。
...
- [c05 算力物理](/kb/基础知识库/c05-算力物理定律与-kv-cache/)
- [c06 SSM / Hybrid](/kb/基础知识库/c06-架构演进-dense-moe-ssm-hybrid/)
- [c08 解码策略](/kb/基础知识库/c08-解码策略与生成控制/)
- [c11 Test-Time Compute](/kb/基础知识库/c11-system-2-思维与-test-time-compute/)
- [KV Cache](/kb/基础知识库/kv-cache/) / [Prompt Caching](/kb/基础知识库/prompt-caching/) / [Attention](/kb/基础知识库/attention/)
- [多模型分层](/kb/基础知识库/多模型分层/) — speculative decoding 的 draft model 是其特例
