...
**Temperature（温度）**：在 [Softmax](/kb/基础知识库/softmax/) 之前对 logit 做缩放。
...
**Logit Bias**：对特定 token 的 logit 直接加减偏置值。设为 -100 可物理禁止某词出现，设为正值可强制倾向某词。[Function Calling](/kb/基础知识库/function-calling/) 的结构化输出依赖此机制（强制模型只能生成指定格式）。
...
**为什么重要**：Prompt Engineering 无法做到 100% 格式保证，只有受约束解码可以。如果业务需要绝对格式合规（[Agent](/kb/基础知识库/agent/) 工具调用、数据提取、API 接口对接），受约束解码是唯一可靠方案。
...
相关概念卡：[Softmax](/kb/基础知识库/softmax/)、[Function Calling](/kb/基础知识库/function-calling/)、[Agent 与工具调用](/kb/基础知识库/agent/)
模块二延伸：[m201 §2.1.1](/kb/工程化与落地架构/m201-prompt-engineering-实战体系/) — Temperature/Top-p 参数在 Prompt Engineering 实战中的配置建议
上一章：[c07 量化](/kb/基础知识库/c07-量化-quantization-与端侧部署/)
下一章：[c09 RAG 架构](/kb/基础知识库/c09-rag-架构/)
