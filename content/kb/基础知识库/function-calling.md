...
Function Calling 依赖 [c08 受约束解码 (Constrained Decoding)](/kb/基础知识库/c08-解码策略与生成控制/)——在解码时动态计算语法合法的 token 集合，保证输出 100% 符合目标函数 schema。

未走受约束解码的"原生 JSON 模仿"会随机崩坏（字段错位、引号不闭合、bool 写成字符串），生产系统必须使用厂商提供的官方 tool_use API 而非自行解析。

## 与 Agent 的关系

Function Calling 是 [Agent](/kb/基础知识库/agent/) 的底层能力。Agent = LLM + Function Calling + ReAct 循环 + Planning + Memory。
...
5. **跨调用上下文丢失**：长 loop 中早期工具返回的关键事实被压出窗口——需要 [Prompt Caching](/kb/基础知识库/prompt-caching/) + 摘要压缩

## 与 MCP 的演化关系

[Function Calling](/kb/基础知识库/function-calling/) 是单个应用与单个 LLM 之间的私有协议；[MCP](/kb/专题-安全对齐与失败/a08-mcp-与-a2a-协议族/) 把它标准化为"客户端-服务端"模式，让任何符合 MCP 的工具可以被任何符合 MCP 的 LLM 客户端使用。
...
- [c08 受约束解码](/kb/基础知识库/c08-解码策略与生成控制/)
- [c10 Agent 核心能力](/kb/基础知识库/c10-agent-技术栈与工具调用/)
- [Agent](/kb/基础知识库/agent/) ReAct [Prompt Caching](/kb/基础知识库/prompt-caching/)
- [MCP](/kb/专题-安全对齐与失败/a08-mcp-与-a2a-协议族/)（Function Calling 标准化的下一步）
- [S01 Agent 六层架构剖面](/kb/专题-安全对齐与失败/s01-agent-六层架构剖面/) §"工具层"
- [Agent 系统化专题](/kb/专题-安全对齐与失败/_agent-系统化专题-总览/)
