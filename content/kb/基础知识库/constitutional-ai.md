...
> [Anthropic](/kb/ai-公司与产品/anthropic/) 2022 年提出的模型对齐方法，简写 CAI。核心思路：用一组明文写下的"宪法原则"（principles / constitution）让模型自我批评并改写自己的回答，再用这些自我改写的数据替代部分人类标注，做 [RLHF](/kb/基础知识库/rlhf/) 的"RL-from-AI-Feedback" (RLAIF) 训练。是当前主流前沿模型对齐路线中"减少人类标注成本 + 让安全规则可读可审计"的代表方案。
...
- **第二阶段：强化学习 (RL-CAI / RLAIF)**
  ...
  - 用 AI 而非人类的偏好作为奖励信号训练 PPO，替代 [RLHF](/kb/基础知识库/rlhf/) 中昂贵的人类标注环节
...
## 与 RLHF 的对比
- **[RLHF](/kb/基础知识库/rlhf/)**：依赖大量人类标注员对成对回答打偏好分。优点是符合"真实人类判断"；缺点是昂贵、慢、标注员一致性低、安全规则隐式不可审计、容易传递标注员自身偏见。
...
- 对齐技术：[RLHF](/kb/基础知识库/rlhf/) DPO RLAIF
- 安全治理：RSP (Responsible Scaling Policy)、Model Cards、AI 公司政治敏感内容立场对比
- 产品议题：m205 - AI 产品形态：从工具到 Agent [p305 - 信任架构与可解释性设计](/kb/产品设计与交互范式/p305-信任架构与可解释性设计/)
- 上下游：c12 - RLHF 与对齐工程（如存在）/ [c14 - 模型评估体系与 Goodhart 陷阱](/kb/基础知识库/c14-模型评估体系与-goodhart-陷阱/)
