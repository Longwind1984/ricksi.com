...
LLM 是有逻辑推理能力的 CPU，[RAG](/kb/基础知识库/rag/) 是外挂的硬盘与实时内存。RAG 的本质是建立**非参数化记忆（Non-parametric Memory）**的调用管线——知识存在检索库中，而非模型权重里。这带来的最大优势是：知识可以实时更新，无需重新训练模型。
...
**密集检索（[Embedding](/kb/基础知识库/embedding/) 向量检索）**：将 query 和文档映射到同一高维向量空间，语义相近即可命中。
- 优势：理解语义变体、模糊查询、跨语言检索
- 劣势：低频词和专有名词的 embedding 质量差（与 [Tokenization](/kb/基础知识库/tokenization/) 直接相关）；黑盒、不可解释
...
相关概念卡：[RAG 检索增强生成](/kb/基础知识库/rag/)、[Embedding 向量嵌入](/kb/基础知识库/embedding/)、[Tokenization](/kb/基础知识库/tokenization/)
模块二延伸：[m203 Embedding 选型与文档解析](/kb/工程化与落地架构/m203-rag-生产环境-embedding-与文档解析/) | [m204 Chunking 策略与 Agentic RAG](/kb/工程化与落地架构/m204-rag-生产环境-chunking-与范式演进/) | [m205 RAGAS 评估体系](/kb/工程化与落地架构/m205-rag-生产环境-索引运维与评估体系/)
上一章：[c08 解码策略](/kb/基础知识库/c08-解码策略与生成控制/)
下一章：[c10 Agent 技术栈](/kb/基础知识库/c10-agent-技术栈与工具调用/)
