...
这个专题的目标是给你一套**经得起反方拷问的判断框架**:不是"AI 越来越强 / 值得关注 / 应该尽早布局"的 hype 风格,而是"Multi-Agent 90% 是 PM 选型陷阱 / MCP 是 LSP 路径复刻不是 REST 对手 / 通用 Agent 70% 上限是数学约束 / Reflexion 工业占比 < 20% / 2026 年默认应该是单 agent"这种**有反方训练的反共识立场**。读完 22 个节点 + 跑通 R01-R03 + 练完 6 题反方对话,你在面试桌上会显著区别于其他转型者——不是因为你知道更多,是因为你能在 30 秒内说清"为什么我不选 X"。
...
依赖关系：概念 → 架构 → 实例 → 复现，代际谱系**横切**这条链路（提供时间维度），阅读指南则是**反向编织**——把 22 个节点编成 3 条可读路径。
...
1. **Round 0**：6 个写作 Agent（W1-W6）并行起草 22 个节点 + 总览 + 阅读指南 + HTML 图谱
...
- **设计来源**:2026-05 Rick 与 Claude 关于"Agent 知识应如何组织"的多轮对话;同期 React 原型对话提出的"紧迫度作为转型者锚点"框架
- **批判性同行评议过程**:见 worktree `agent-knowledge-validation/critiques/`
- **HTML 图谱可视化**:见同目录 `knowledge-graph.html`,是本总览的**图形版本**,节点点击可看一句话简介
