...
### 体验影响（着重 · 先看）
- **首页 7 张项目卡的综述全部重写**：从平均 ~110 字的密集长段，砍到 1-2 句钩子（最短 52 字、最长 112 字），整体减约 40-65%。卡片高度从原先不等压到统一的 203-214px，扫读更轻。
- **AI 味来源已定位并清除**：用 Rick 自己的 Writer Pipeline（`/Users/rick/code/Writer`）critic 反 AI 味量表诊断，命中四类——金句模板堆砌（「不是 A，是 B」普适格言）、破折号 `——` 堆砌、报菜名（数字目录）、节奏单一。改写后卡片综述零 `——`、每条至多 1 记 earned 判断。
- **信息没丢，只是下沉**：删掉的关键决策/取舍/全部具体数字（123 变量、2.2万 draw call、28/28 绿、277×11 字段等）按计划留给落地页详细介绍——**那一阶段等用户确认本批卡片后再做**。
- **范围严格限定**：只动 `src/data/projects.ts` 的 7 条 `desc` 与顶部文案纪律注释；落地页 `src/pages/projects/*.astro`（各自有独立 desc 常量）、搜索索引（只用 title/tags）均未受影响。

### 做了什么
把首页项目卡片里冗长、AI 味重的"综述"逐条改短改顺。先用 Rick 已有的写作系统当尺子扫出问题，再按"钩子档"（用户在三档力度里选定）重写 7 条。dev 服务器实测：7 条新文案全部正确渲染、无控制台/服务端报错；桌面 1440 与窄屏实测零横向溢出、零文字截断。

### 关键决策与被否决
- **改写力度选「钩子化」**：给了轻修(~15%)/中度(~40%)/钩子(~60-65%) 三档实物前后对比，用户选最激进的钩子档。代价是卡片不再逐条交代"角色/决策/取舍/结果"，这些移交落地页。
- **用 Writer Pipeline 而非通用文案技能**：用户指定用本地 Writer 项目的 critic 量表做诊断依据，结论与该项目自己卡片写的"AI 味在『太像我自己』的金句口癖"自洽。
- **文案纪律注释同步改写**：旧注释要求"每条交代角色/决策/取舍/结果"与钩子化冲突，改为"只留 1-2 句钩子，数字下放落地页"，避免文档与代码现状打架。
- **数字保守取整不杜撰**：仅保留原文已有数字；`3,225 篇`表述为「三千多篇」；其余删减项原样留给落地页。

### 当前状态：能跑什么
- `cd rick-homepage && npm run dev` → 首页 `#projects` 段：4 张 featured 卡 + 展开后 3 张，均显示新综述。dev 实测通过。
- `npm run build`：本会话末尾后台运行中（慢，因 ~1318 张分享图构建期生成，与本次纯文本改动无关）。改的是字符串字面量，dev 同款编译已通过。

### 未尽事项与已知问题
- **落地页详细介绍尚未推广**：等用户确认本批卡片综述后，再把同一套去 AI 味原则用到 `src/pages/projects/*.astro`。
- **预览截图工具在本环境失效**：该玻璃态站点 headless 截图返回纯深色帧（backdrop-filter），故本次用 DOM eval（文本/几何测量）替代视觉截图验证。
- **WeatherLens 卡标题行有 ~26px 内部溢出**（图标+标题+「仓库整理中」标签），是既有问题、与本次 desc 改动无关，未在页面级造成横向滚动；记录备查。

### 文件级变更清单
- `src/data/projects.ts`：7 条 `desc` 全部重写为钩子档；顶部文案纪律注释（行 3-4）改为钩子化规则。其余字段（title/tags/icon/img/href/featured）未动。
...
## 2026-07-23 · 装置标签补齐与历史入口文案收束

### 做了什么

首页三张装置卡现在拥有同一层级的能力标签：知识图谱 Galaxy View 标记「知识图谱 / 3D 可视化 / 交互漫游」，AI 前沿追踪标记「人物追踪 / 事件分级 / 星图时间流」；历史全景知识台保留「时间轴 / 地图 / 关系网络」。同时删除历史卡第二段“从一个事件出发……”说明，减少重复解释，保留一段主文案直接交代产品定位。

### 关键决策与被否决的备选

- 标签只描述用户能实际体验的能力，不增加“AI 驱动”“沉浸式”等泛化宣传词。
- 不改卡片结构、SVG、字号或响应式布局；本轮是信息层级补齐，不借小文案改动重新设计已确认的装置区。

### 当前状态：现在能跑什么、怎么跑

- `node --test test/*.test.mjs`：18/18 通过。
- `npm run build`：685 个页面约 11.6 秒；社交图 `generated=0 reused=1667`，仅改首页没有重画知识库或前沿卡片。
- 构建产物已确认六个新增标签存在，待删除句子不存在；`git diff --check` 通过。

### 未尽事项与已知问题

- 生产发布后仍需在桌面与移动端实看三张卡的标签换行，并确认历史入口继续指向 `https://history.ricksi.com/`。
- 既有生产依赖安全公告不在本轮文案改动范围内，仍需单独安排大版本升级与回归。
...
## 2026-07-28 · 公众号旧文迁移：来源反链底座与微信读取阻塞

### 做了什么

在独立分支为博客文章增加可选的公众号来源元数据。迁移文章进入现有 `/blog` 内容体系后，会继续复用本站的窄栏阅读、
移动端排版、导航与分享卡，并在文章日期下方显示一个克制的“原始发布”胶囊，直接反链公众号原文。README 已补充正文
与图片的迁移约定：图片必须转存到站内资产目录，不长期热链微信 CDN。

### 关键决策与被否决的备选

- 保持本站 canonical，公众号链接作为原始发布来源与回访入口；没有复制微信页面样式，避免文章在站内形成第二套视觉。
- 为真实的多篇迁移增加 `sourceUrl` / `sourceName` 两个正交字段，不引入导入框架或额外依赖。
- 被否决：根据标签页标题或站内现有文章猜测正文。微信短链的正文、发布日期、公众号名和图片均未被可靠读取。
- 被否决：在浏览器安全策略明确禁止读取后，改用隐藏脚本、其他浏览器或代理绕过。

### 当前状态：现在能跑什么、怎么跑

- `VERCEL=1 npm run build`：通过，内容解析、类型生成、客户端打包、689 个知识库页面与全部现有博客页完成构建；
  Vercel 模式继续沿用运行时按需生成社交图，完整构建约 96 秒。
- 临时公众号预览文章曾完成页面预渲染与 OG 图生成，验证来源字段分支可编译；预览稿已删除，不会混入交付内容。
- `git diff --check`：通过。

### 未尽事项与已知问题

- 《哥伦比亚 54 天，没待够，没读懂，没写完》的真实正文、发布日期、公众号名和原图仍未迁入。Chrome 与 ChatGPT
  内置浏览器都能识别用户已打开的目标标签页，但浏览器安全策略禁止读取该微信 URL；公开搜索也没有可核证索引。
- 继续迁移需要公众号后台的一次性文章导出/素材包，或其他由用户直接提供的正文与原图输入。取得输入后再批量生成
  `content/posts/*.md` 与 `public/assets/blog/<slug>/`，做桌面/移动端视觉验收。
- EdgeOne 纯静态模式在本机无社交图缓存时会线性重画数百张知识库分享图，首轮已因成本过高主动中止；本次改动未触及
  该既有构建策略，也未降低验收标准。
...
## 2026-07-28 · 公众号旧文迁移续：两篇拉美行记与原图落站

### 做了什么

根据 Downloads 中由浏览器“保存完整网页”得到的公众号页面，恢复并迁入《哥伦比亚 54 天，没待够，没读懂，没写完》
与《洪都拉斯 17 天，香蕉共和国行记》。两篇文章进入现有 `/blog` 列表和详情页，保留原文段落、图片顺序、发布日期与
公众号出处；共 21 张正文图已从微信图片源直接取得无水印版本并转存站内，每篇都能从来源胶囊反链回“大笑且歌唱”原文。

### 关键决策与被否决的备选

- 复用本站既有玻璃卡片、窄栏正文、响应式排版和分享能力；公众号仅作为内容来源，不复制其正文 CSS，也不形成第二套视觉。
- 微信图片只作为一次性素材源，正式页面引用本站静态资产，避免防盗链、签名变化或外部图片下线。
- 洪都拉斯原图最高 2560px；大于 1600px 的图片以 JPEG quality 92 渐进式缩放，小图保持原文件。正文图片总量约
  8.0 MB，但全部懒加载并写入固有尺寸，兼顾高密度屏幕清晰度、布局稳定与首屏成本。
- 被否决：使用保存网页目录中带“公众号 · 大笑且歌唱”水印的 640/1080px 副本。保存页内同时保留了原始微信图片地址，
  可直接取得更高质量无水印版本。
- 被否决：远程热链微信 CDN。短期少一次资源复制，但可靠性与本站性能控制均不可接受。

### 当前状态：现在能跑什么、怎么跑

- `/blog/colombia-54-days/`：正文 9 节、11 张图、公众号反链。
- `/blog/honduras-17-days/`：正文 9 节、10 张图、公众号反链。
- 浏览器实测桌面 1280px 与移动端 390×844；两页均无横向溢出，21 张懒加载图片逐张完成，控制台无 error / warning。
- `npm run verify:privacy`：全部隐私不变量通过。
- `VERCEL=1 npm run build`：两篇文章、博客索引及全站构建通过。
- `git diff --check`、构建产物图片引用数、来源反链、无微信 CDN 热链检查：通过。

### 未尽事项与已知问题

- 目前 Downloads 里只有这两篇可确认属于“大笑且歌唱”的完整公众号保存页；单篇保存页不包含公众号历史文章索引。
  后续文章仍需继续“网页，全部”保存到 Downloads，迁移可以沿用同一结构批量追加。
- 当前分支只在本地完成，尚未 push、创建 PR 或部署；发布前需要用户确认视觉草案与公开范围。
