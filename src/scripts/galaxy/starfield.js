...
// 移植自 galaxy-view src/render/starfield.ts —— 剥 TS 类型，逻辑不变
const CLASSES = [
  { count: 2600, size: 1.2 },
  { count: 900, size: 2.0 },
  { count: 250, size: 3.0 },
...
export function disposeStarfield(group) {
...
export class Twinkler {
  constructor(geometry, starCount) {
    this.geometry = geometry;
    this.starCount = starCount;
    this.active = null;
    this.nextIn = 3;
    this.attr = geometry.getAttribute('color');
    this.baseColors = new Float32Array(this.attr.array);
  }
...
export function buildStarfield(shellRadius, scale = 1) {
  ...
  return {
    ...
    twinkler:
      twinkler ??
      new Twinkler(new BufferGeometry().setAttribute('color', new BufferAttribute(new Float32Array(3), 3)), 1),
  };
}
